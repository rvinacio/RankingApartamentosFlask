...
# def remover_imovel():
#     data = request.get_json()
#     print("Dados recebidos do JS:", data)  # Log dos dados recebidos do JS
#     fonte_list = data.get('codigo')  # fonte_list agora contém uma lista de códigos
    
#     if not fonte_list:
#         return jsonify({'status': 'error', 'message': 'Lista de fontes não fornecida'}), 400
    
#     # Log da lista de fontes recebida
#     print("Lista de fontes recebida:", fonte_list)

#     # Atualizar o ranking removendo os apartamentos
#     atualizacao_sucesso = atualizar_ranking_apartamentos(fonte_list)
    
#     # Se a atualização foi bem-sucedida, passar para a função de desconsiderar o apartamento
#     if atualizacao_sucesso:
#         desconsideracao_sucesso = desconsiderar_apartamento(fonte_list)
        
#         if desconsideracao_sucesso:
#             return jsonify({'status': 'success', 'message': 'Apartamentos removidos e desconsiderados com sucesso.'}), 200
#         else:
#             return jsonify({'status': 'error', 'message': 'Falha ao desconsiderar apartamentos.'}), 500
#     else:
#         return jsonify({'status': 'error', 'message': 'Falha ao atualizar o ranking de apartamentos.'}), 500
